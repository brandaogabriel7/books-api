...
@pytest.mark.parametrize("description", ["this is a book description", "this is another book description"])
def test_createBook_sucessWithDescription(description: str):
  book = Book("123", "Title", description)
  assert book.id == "123", "Id should be '123'"
  assert book.title == "Title", "Title should be 'Title'"
  assert book.description == description, "Description should be 'description'"

@pytest.mark.parametrize("description,", ["new description", "other description"])
...
def test_removeAuthors():
  authors = ["Author name", "Another author name", "One more author name", "Last author name"]
